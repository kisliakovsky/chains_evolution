...
def save_for_kirill(sequences: Dict[str, List[str]], step_name: str):
    _save_csvs(sequences, "forKirill", step_name, by_clusters=False)
...
def _save_csvs(sequences: Dict[str, List[str]], file_name: str, step_name: str, by_clusters: bool):
    ...
    node_file_path = paths.build_output_table_path(section_path, "nodes".format(file_name))
    edge_file_path = paths.build_output_table_path(section_path, "edges".format(file_name))
